...
\begin{frame}\frametitle{BASIC MONOIDS IN PROGRAMMING}
  \begin{itemize}
    ...
    \item<3-> \texttt{Set[A]} under \texttt{union} with \texttt{Set()}.
  \end{itemize}
...
\begin{frame}[fragile] \frametitle{A MONOID FOR FUNCTIONS}
  \texttt{Monoid[A => B]} where \alert{\texttt{B} has a Monoid}
...
\begin{frame} \frametitle{LARGER USE CASES}
  ...
  \begin{block}{Weighted average of the means}
    ...
    \begin{split}
      ...
      \frac{1}{n+m} \sum_{i=0}^{n+m-1} x_i = \frac{\alert{n}\mu + \alert{m}\nu} {\alert{n+m}}
    \end{split}
    \]
  \end{block}
